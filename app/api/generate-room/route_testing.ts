...
const MAX_GENERATION_REFERENCE_IMAGES = 6; // B3: reduced to 6 — fewer focused references beats 14 diverse ones
...
Task:
Redesign this ${roomType.replaceAll("_", " ")} by placing the catalogue furniture into the BASE ROOM IMAGE shown immediately after this text.

CRITICAL — image order and geometry rules:
The first image you receive (BASE ROOM IMAGE) defines the architectural container. Its walls, floor, ceiling, windows, curtains, doors, skirting boards, camera angle, perspective, and room proportions are ALL FIXED and must appear identically in your output. Do not adopt the backgrounds of the catalogue product photos — those are studio shots on neutral backgrounds. Your room architecture comes from the BASE ROOM IMAGE only.
Think of your task as: take the catalogue furniture objects and composite them into the BASE ROOM photograph. The room does not change. Only furniture changes.
...
Strict room-preservation rules (these override everything):
12. The BASE ROOM IMAGE geometry is fixed. Your output must have identical room proportions, ceiling height, wall positions, floor material, and spatial layout to the BASE ROOM IMAGE.
13. Preserve walls, windows, curtains, doors, ceiling, floor material and colour, skirting, trims, camera angle, and perspective exactly. Do not add, remove, or reposition any architectural element.
14. Do not change the floor — wood stays wood, tiles stay tiles, concrete stays concrete.
15. Do not adopt backgrounds from catalogue product photos. Those are studio environments. Your background is always the BASE ROOM IMAGE.
16. Add or replace only movable furniture and decor. Never alter architecture.
17. If you cannot place a catalogue product without changing the room architecture, omit that product rather than altering the room.
18. Keep the output photorealistic and consistent with the user's requested style.
19. Keep the room type correct.
20. Use shortlist objects with strong visual alignment to the references.
21. If the user asks for color/material/style changes, apply those to shortlist-aligned products rather than switching to unrelated products.
${editText}
${targetedEditText}
${retryText}
...
function buildRecommendationRankingPrompt(
  ...
  return `
You are a visual product matching expert. Your job is to match each visible furniture piece in the room to the most similar catalogue product shown in the reference images.

You will be shown:
- Image 1: the AI-generated room (furniture was generated by an AI, not placed from the catalogue)
- Images 2 onwards: catalogue product reference images, each labelled with its product_handle

CRITICAL: The room was generated by an AI inpainting model. The furniture in the room will NOT exactly match the catalogue products. Your job is purely visual similarity matching:
- Look at each visible furniture piece in the room
- Find which catalogue reference image looks most similar in: shape, colour, material, and category
- A beige L-shaped sectional → match to the closest beige/neutral sofa in the catalogue
- A wooden slatted coffee table → match to the closest wooden coffee table in the catalogue
- Never return a product from a different category (do not match a sofa to a chair)
- Never return a product that is genuinely not visible in the room
...
Matching rules:
1. For each major visible furniture piece in the room (sofa, chair, coffee table, rug, lamp, cabinet, etc.), find the catalogue reference image that most closely matches it in terms of category, colour, material, and style.
2. A match does not need to be visually identical — match by category first, then by closest style/colour. A grey sofa in the room matches the closest grey sofa in the catalogue references.
3. For each furniture category visible in the room, return exactly ONE best-matching catalogue product_handle. Do not return multiple handles for the same category.
4. If a visible furniture category has NO catalogue reference at all (e.g. a piano when no piano reference was provided), mark it as innovative.
5. Do not mark anything as innovative if a catalogue reference exists for that furniture category — always prefer the closest catalogue match.
6. For innovative items, provide a tight bbox [x1,y1,x2,y2] in 0-1000 coordinates.
7. Never return a catalogue product_handle for furniture that is genuinely not visible in the room.

Catalogue product handles available:
${shortlistSummary(shortlist)}
...
function isStrongEnoughToStop(v: FurnishedRoomValidation, score: number) {
  return (
    ...
    v.structural_drift_level !== "significant" &&
    v.structural_drift_level !== "moderate" &&
...
// B3: Primary furniture slots — these get priority in the 6-image reference budget.
// Order matters: Gemini pays most attention to the first images it sees.
const PRIMARY_FURNITURE_SLOTS = ["sofa", "chair", "table", "rug", "lamp", "cabinet", "bed", "desk", "mirror"];

function selectFocusedReferences(
  shortlist: ShortlistItem[],
  authoritativeSelection: AuthoritativeSelectionItem[],
  theme: string,
  maxRefs: number
): ShortlistItem[] {
  // Start with authoritative selection — these are the highest-confidence picks
  const authHandles = new Set(authoritativeSelection.map((x) => x.product_handle));
  const result: ShortlistItem[] = [];
  const usedCategories = new Set<string>();
  const usedHandles = new Set<string>();

  // Pass 1: One authoritative pick per primary furniture slot
  for (const slot of PRIMARY_FURNITURE_SLOTS) {
    if (result.length >= maxRefs) break;
    const match = authoritativeSelection.find((item) => {
      if (!item.product_handle || usedHandles.has(item.product_handle)) return false;
      const cat = canonicalizeCategory(item.normalized_category || item.category || item.bucket || "");
      return cat === slot || (item.requestedCategory && canonicalizeCategory(item.requestedCategory) === slot);
    });
    if (match) {
      result.push(match);
      usedHandles.add(match.product_handle);
      usedCategories.add(canonicalizeCategory(match.normalized_category || match.category || match.bucket || ""));
    }
  }

  // Pass 2: Fill remaining slots from shortlist, one per uncovered category
  if (result.length < maxRefs) {
    const scored = prioritizeShortlist(shortlist, theme, maxRefs * 3);
    for (const item of scored) {
      if (result.length >= maxRefs) break;
      if (!item.product_handle || usedHandles.has(item.product_handle)) continue;
      const cat = canonicalizeCategory(item.normalized_category || item.category || item.bucket || "");
...
// For detection: use the FULL shortlist (up to MAX_PINNED_DETECTION_REFERENCE_IMAGES)
// More diverse references = better chance of finding a visual match for what Imagen generated.
// Unlike generation (which needs focused refs to avoid confusion), detection benefits from breadth.
async function buildDetectionReferenceParts(shortlist: ShortlistItem[], authoritativeSelection: AuthoritativeSelectionItem[], theme: string) {
  // Use ALL shortlist items, prioritising authoritative picks first, up to detection cap
  const authHandles = new Set((authoritativeSelection || []).map((x) => x.product_handle));
  const sorted = [
    ...shortlist.filter((x) => authHandles.has(x.product_handle)),
    ...shortlist.filter((x) => !authHandles.has(x.product_handle)),
  ].filter((x) => !!x.image_url).slice(0, MAX_PINNED_DETECTION_REFERENCE_IMAGES);
  ...
  const resolved = await Promise.all(
    sorted.map(async (item) => {
      try {
        ...
        if (status !== "404") {
          console.warn("Skipping detection image:", item.image_url, status ?? String(err));
        }
...
async function buildReferenceParts(shortlist: ShortlistItem[], authoritativeSelection: AuthoritativeSelectionItem[], theme: string) {
  // B3: Use focused reference selection — one per furniture category, prioritising
  // authoritative picks. This beats sending 14 diverse images where Gemini gets confused.
  const prioritized = selectFocusedReferences(
    shortlist,
    authoritativeSelection?.length
      ? authoritativeSelection
...
async function generateRoomAttempt(
  ...
  // Room image comes FIRST so Gemini anchors to the room geometry before
  // seeing product references. Product backgrounds otherwise dominate.
  const roomParts: any[] = [];
  if (editMode && baseGeneratedImage) {
    roomParts.push({ text: "BASE ROOM IMAGE (this defines the exact architecture, geometry, floor, walls, windows, and camera angle — do not alter any of these):" });
    roomParts.push({
      inlineData: {
  ...
  const parts: any[] = [
    { text: prompt },
    ...roomParts,
    { text: "CATALOGUE PRODUCT REFERENCE IMAGES (place these furniture items into the room above — use the room geometry, not the product photo backgrounds):" },
    ...referenceParts,
  ];
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Detection-only mode ───────────────────────────────────────────────────
    // When detectionOnly=true, skip room generation entirely.
    // Just run detectPinnedProducts on the provided baseGeneratedImage.
    // Used after Imagen inpainting — the room image is already generated,
    // we only need to find which catalogue products match what is visible.
    const detectionOnly = !!body.detectionOnly;

    if (detectionOnly) {
      if (!baseGeneratedImage) {
        return NextResponse.json({ ok: false, error: "baseGeneratedImage is required for detectionOnly mode" }, { status: 400 });
      }
      if (!theme?.trim()) {
        return NextResponse.json({ ok: false, error: "theme is required" }, { status: 400 });
      }
      if (!mergedShortlist.length) {
        return NextResponse.json({ ok: false, error: "shortlist is required" }, { status: 400 });
      }
      // Note: cleanedRoomBase64 is NOT required in detectionOnly mode — only baseGeneratedImage matters

      const detectionImageBase64 = stripDataUrlPrefix(baseGeneratedImage);
      const detectionMimeType = mimeType;

      // Build detection reference parts — use FULL shortlist for maximum match coverage
      const effectiveSelection = ensureSelectionCoverage(mergedShortlist, authoritativeSelection, theme || "");
      const { catalogueImageParts, validShortlist } = await buildDetectionReferenceParts(
        mergedShortlist,
        effectiveSelection,
        theme || ""
      );

      const detectionEditPlan = buildServerEditPlan(theme || "", false, body.editPlan, []);

      // Debug: log what products are being sent for detection
      console.log("Detection shortlist (" + validShortlist.length + " products):");
      validShortlist.slice(0, 16).forEach((item, i) => {
        console.log(`  ${i+1}. [${item.normalized_category || item.category || item.bucket}] ${item.title?.slice(0, 60)}`);
      });
      ...
      console.log("Pinned products detected:", pinnedProducts.map(p => p.title?.slice(0, 40)));
      const sceneState = buildSceneState(pinnedProducts, detectionEditPlan);

      return NextResponse.json({
        ok: true,
        generatedImage: baseGeneratedImage, // return the same image unchanged
        pinnedProducts,
        selectedProducts: effectiveSelection,
        validationPassed: true,
        editPlan: detectionEditPlan,
        sceneState,
      });
    }
    // ── End detection-only mode ───────────────────────────────────────────────
